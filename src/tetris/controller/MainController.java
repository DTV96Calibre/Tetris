...
public class MainController {
    ...
    public static final int INPUT_MOVE_LEFT = Input.KEY_LEFT;
    public static final int INPUT_ROTATE_TETRIMINO_LEFT = Input.KEY_UP;
    ...
    public void render(GameContainer gc, StateBasedGame s, Graphics g) {
        ...
        if (s.getCurrentStateID() == 2) {
            // render the GameOver screen components over the gameboard
            int widthOffset = 0;
            int heightOffset = Window.getHEIGHT() / 3;
            int animationWidth = Window.getWIDTH() / 2;
            int animationHeight = animationWidth / 4;
            theView.getGameOverState().getGameOverAnimation().draw(widthOffset,
                                                                   heightOffset,
                                                                   animationWidth,
                                                                   animationHeight);
        }
    ...
    public void update(GameContainer gc, StateBasedGame s, int delta) {
        ...
        if (s.getCurrentStateID() == 0) {
            ...
            if (input.isKeyPressed(INPUT_ROTATE_TETRIMINO_LEFT)) {
                rotateActiveTetrimino(1);
            } // if user pressed the left arrow key, try to move the tetrimino left
            else if (input.isKeyPressed(INPUT_MOVE_LEFT)) {
                moveActiveTetrimino(Direction.LEFT, s);
            } // if user pressed the right arrow key, try to move right
            else if (input.isKeyPressed(INPUT_MOVE_RIGHT)) {
                moveActiveTetrimino(Direction.RIGHT, s);
            } else if (input.isKeyDown(INPUT_SOFT_DROP)) {
    ...
    public void updateActiveTetrimino(GameContainer gc, StateBasedGame s,
        ...
        if (timer < theModel.getGameSpeed()) {
            ...
            moveActiveTetrimino(Direction.DOWN, s); // drop tetrimino by 1 space
        }
    }
     ...
     * @param s The game state object allowing us to transition to the menu
     * @return a boolean indicating whether this Tetrimino was able to be moved
     */
    public boolean moveActiveTetrimino(Direction d, StateBasedGame s) {
     ...
     * @param s The game state object allowing us to transition to the menu
     */
    public void instantDropTetrimino(StateBasedGame s) {
        // move the Tetrimino down, space-permitting
        while (moveActiveTetrimino(Direction.DOWN, s)) {
     ...
     * @param s The game state object allowing us to transition to the menu
     */
    public void checkGameOver(StateBasedGame s) {
